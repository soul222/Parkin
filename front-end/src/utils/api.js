...
class ApiClient {
  ...
  async request(endpoint, options = {}) {
    ...
    try {
      ...
      if (response.status === 403) {
        const msg = "Maaf, akses ditolak";
        useToastStore().addToast({ message: msg, type: 'error' });
        throw new Error(msg);
      }

      if (!response.ok) {
        const msg = data.message || `Terjadi kesalahan pada sistem`;
        useToastStore().addToast({ message: msg, type: 'error' });
        throw new Error(msg);
      }
  ...
  async #handleTokenExpired(originalUrl, originalConfig) {
    ...
    try {
      ...
      const msg = "Sesi berakhir. Mohon login kembali.";
      useToastStore().addToast({ message: msg, type: 'warning' });
  ...
  async #handleNetworkError(method, url, originalError) {
    ...
    if (["POST", "PUT", "DELETE"].includes(method)) {
      const msg = "Anda sedang offline. Perubahan akan disinkronkan saat koneksi kembali.";
      useToastStore().addToast({ message: msg, type: 'warning' });
      throw new Error(msg);
    }
